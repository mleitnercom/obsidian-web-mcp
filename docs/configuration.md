...
| Variable | Default | Description |
|---|---|---|
| `VAULT_CREATE_NOTE_PATH_PATTERN` | empty | Regex the path must fully match; empty allows any `.md` path |
...
| `VAULT_CREATE_NOTE_MAX_BYTES` | `0` | Per-note ceiling for create-only writes; `0` means `VAULT_MAX_CONTENT_SIZE` |
